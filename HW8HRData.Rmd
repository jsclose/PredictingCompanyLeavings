...
Our logistic model was fairly accurate. This stemmed from having multiple strong 3 star variables. One of the strongest indicator variables was satisfaction level. This intuitively makes sense because if someone is unhappy with their job they would want to leave and vice-versa.
...
One of the SVM models, the rbfdot model worked very well and had an accuracy of 92%. The vanilladot model on the otherhand worked very poorly and only produced a 47% accuracy. This is interesting and shows that we should be using the rbfdot model for a better predicter if we want to use SVM. 
```
...
cv_results <- lapply(folds, function(x) {
  hr_train <- hr[x, ]
  hr_test <- hr[-x, ]
  hr_model <- C5.0(default ~ ., data = hr_train)
...
# Conclusion
```{r}
acc_predictions = c(a1,a2,a3,a4,a5, a6, a7)
...
names = c("Log Prediction Plain", "SVM with Vanilla Kernal","SVM Radial Basis Kernal","Decision C5.0 Tree Regression","Decision Tree C5.0 Regression trails 10", "Decision Tree C5.0 with Cost Model", "Random Forest")
